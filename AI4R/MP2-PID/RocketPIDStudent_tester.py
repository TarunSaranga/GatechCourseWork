...
class RocketPID:
    ...
    def launch_rocket(self, rocket_pid_solution, show_graph=True):
        ...
        for instance in range(len(time) - 1):
            ...
            self.rho = 1225000000 * np.exp(-height[instance] / 1000)  # simulate air density drop with altitude
        ...
        flight_score = (min(score_one,90) + min(score_two,40)) / 2

        output += '\n'
        output += 'Optimal Flight Score: {}\nLanding Score: {}\n'.format(flight_score, score_landing)
        output += '\n'
        output += 'Part B Completion: {}%\n'.format(min((score_one + score_two) / 2 + score_landing, 100))
        output += 'Points Earned: {}\n'.format((flight_score+ score_landing) * 0.75, 75.0)
...
class PIDTest(unittest.TestCase):
    ...
    def test_running_pumps(self):
        ...
        self.assertEqual(pressure_score, 100, 'Turbopump pressure maintained and did not exceed design limits.')
