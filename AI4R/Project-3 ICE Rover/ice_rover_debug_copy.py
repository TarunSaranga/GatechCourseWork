    ...
    def next_move(self, sample_todo, measurements):
        ...
        if(self.sam_one == 0):
            ...
            if(len(self.OS.siteids)>0):
                ...
                steering = bearing_to_site#self.OS.truncate_angle(bearing_to_site - self.OS.bearing)
                
                steering = max(-self.max_steering, steering)
                steering = min(self.max_steering, steering)
                #print("Steering",steering)
                #print("")
                if(steering < 0.00000001 and steering > -0.00000001):
