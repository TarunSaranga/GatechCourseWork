...
#def reshape(q,x,y):
#    m = [[0 for row in range(x)] for col in range(y)]
#    k=0
#    for i in range(y):
#        for j in range(x):
#            m[i][j] = q[k]
...
def move(p,motion):
    ...
    for i in range(len(p)):
        for j in range(len(p[i])):
            q[i][j] = (p_move* p[(i-motion[0])%len(p)][(j-motion[1])%len(p[i])]) + ((1.0 - p_move)*p[i][j])
...
def sense(p, colors, measurement):
        ...
        for i in range(len(p)):
            for j in range(len(p[i])):
                ...
                q[i][j] = p[i][j] * (hit*sensor_right +(1.0 - hit)*sensor_wrong)
                s+=q[i][j]
